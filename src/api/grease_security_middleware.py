...
def _require_token() -> bool:
    ...
    Para laboratório/dev/testes, defina GREASE_REQUIRE_TOKEN=false para desativar.
    """
    return os.getenv("GREASE_REQUIRE_TOKEN", "true").strip().lower() not in _DISABLE_VALUES
...
def _client_ip(
    ...
    if os.getenv("GREASE_TRUST_X_FORWARDED_FOR", "").strip().lower() in {"1", "true", "yes", "on"}:
        if forwarded_for:
...
def validate_grease_ingest_security(
    request: Request,
    ...
    allowed_ips_raw = os.getenv("GREASE_ALLOWED_SOURCE_IPS", "")
    ...
    if not expected:
        ...
        # (a menos que GREASE_REQUIRE_TOKEN=false, apenas para laboratório/dev).
        if _require_token():
            raise HTTPException(
                status_code=503,
