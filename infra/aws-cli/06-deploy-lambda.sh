...
if aws lambda get-function \
  ...
  aws lambda create-function \
    ...
    --timeout 30 \
    --memory-size 256 \
    --environment "Variables={RAW_BUCKET=${RAW_BUCKET},ENABLE_TIMESTREAM=${ENABLE_TIMESTREAM},TIMESTREAM_ENABLED=${TIMESTREAM_ENABLED},TIMESTREAM_DB=${TIMESTREAM_DB},TIMESTREAM_TABLE=${TIMESTREAM_TABLE},DYNAMODB_TABLE=${DYNAMODB_TABLE},EVENT_BUS=${EVENT_BUS}}" \
    --region "$AWS_REGION" \
    --profile "$AWS_PROFILE" >/dev/null
