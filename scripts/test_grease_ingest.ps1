...
if (-not $Endpoint) { $Endpoint = "http://127.0.0.1:8000/grease/ingest" }

$Headers = @{ "Content-Type" = "application/json" }
if ($env:GREASE_INGEST_TOKEN) { $Headers["X-API-Key"] = $env:GREASE_INGEST_TOKEN }
...
$Payload = @{
  ...
  cycle_id = "cycle_test_" + (Get-Date -Format "yyyyMMdd_HHmmss")
  metrics = @{
  ...
  quality = @{ source = "powershell_test"; sensor_range_bar = 250 }
}
